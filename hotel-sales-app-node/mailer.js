/* mailer.js -- automatic client emails.
   ...
   Sends over Resend's HTTPS API rather than raw SMTP. Free hosts like
   Render block outbound SMTP ports (25/465/587) on their free tier to
   stop spam relaying, which makes nodemailer/SMTP fail with a connection
   timeout no matter how the credentials are set. An HTTPS API call goes
   over port 443 like any normal web request, so it isn't affected.

   All of this is best-effort: if RESEND_API_KEY isn't set, or a send
   fails, the app logs it and carries on -- email is never allowed to
   block or break the ledger/calendar features. */

const HOTEL_NAME = process.env.HOTEL_NAME || 'our sales desk';
const RESEND_API_KEY = process.env.RESEND_API_KEY || '';
// onboarding@resend.dev works with no setup, but Resend only lets that
// address send to the email you signed up with -- fine for testing, but
// for real guest emails you need to verify your own domain on Resend and
// point MAIL_FROM at an address on it, e.g. sales@yourhotel.com.
...
if (!RESEND_API_KEY) {
  console.warn(
    'Email is not configured (missing RESEND_API_KEY) -- automatic ' +
    'inquiry and follow-up emails will be skipped. See .env.example.'
  );
}
...
async function sendMail({ to, subject, html, text }) {
  if (!RESEND_API_KEY) return { ok: false, error: 'Email is not configured on this server.' };
...
async function runFollowUpSweep(getPayload, savePayload) {
  if (!FOLLOWUP_ENABLED || !RESEND_API_KEY) return;
  let rows;
  try {
