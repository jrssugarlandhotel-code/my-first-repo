...
// 60mb gives headroom for RAW camera photos (e.g. .dng), which are much
// bigger than regular compressed JPEGs and grow ~33% again once base64-encoded.
...
// Client-facing automatic emails. Two kinds:
//   kind: 'inquiry-received' -- fired once by the frontend right after a
//     guest submits the Event Inquiry Form (immediate thank-you note).
//   kind: 'manual-followup'  -- fired by the "Send follow-up now" button
//     in the ledger entry modal; also stamps followUpSentAt on that row
//     so the automatic sweep won't send a second one later.
// The automatic reminder sweep itself (no button needed) runs in the
// background -- see mailer.startFollowUpSweep() below.
app.post('/api/notify', async (req, res) => {
  const body = req.body || {};
  const { kind, to, name, title, eventDate, execName, ledgerId } = body;

  if (kind === 'inquiry-received') {
    const { subject, html, text } = mailer.inquiryReceivedEmail({ name, title, eventDate, execName });
    const result = await mailer.sendMail({ to, subject, html, text });
    return res.json(result);
  }

  if (kind === 'manual-followup') {
    const { subject, html, text } = mailer.followUpEmail({ name, title, eventDate });
    const result = await mailer.sendMail({ to, subject, html, text });
    if (result.ok && ledgerId) {
      try {
        const rows = await getPayload('ledger');
        ...
        console.error('Could not stamp followUpSentAt for', ledgerId, '-', e.message);
      }
    }
    return res.json(result);
  }

  res.status(400).json({ ok: false, error: 'Unknown notify kind' });
});
...
initDb()
  .then(() => {
    ...
    mailer.startFollowUpSweep(getPayload, savePayload);
  })
  .catch((e) => {
